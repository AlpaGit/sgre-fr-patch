...
$ExpectedMotionInfoHash = 'FC429D5559A1733807EA162741572AEB47BFDE2DD564FAF0B897F5027405CB02'
$ExpectedMotionBodyHash = '76B6A1BA0EA548DA301AFFFD6C52F502D57FBD17F9C9048D2A7CE4B5334B022A'
...
Write-Host "  scenario, d'interface et les trois videos, puis installer la VF." -ForegroundColor Gray
...
$frMotionInfo = Join-Path $patchDir 'motion_info.psb.m'
$frMotionBody = Join-Path $patchDir 'motion_body.bin'
...
foreach ($file in @($frInfo, $frBody, $frMotionInfo, $frMotionBody) + @($frMovies.Values)) {
...
if ((Get-FileHash -Algorithm SHA256 -LiteralPath $frMotionInfo).Hash -ne $ExpectedMotionInfoHash -or
    (Get-FileHash -Algorithm SHA256 -LiteralPath $frMotionBody).Hash -ne $ExpectedMotionBodyHash) {
    Write-Fail "Les fichiers d'interface sont incomplets ou corrompus."
    Write-Host "  Telechargez de nouveau l'archive depuis GitHub Releases." -ForegroundColor Red
    Pause-And-Exit 1
}
foreach ($name in $ExpectedMovieHashes.Keys) {
...
foreach ($file in @($targetInfo, $targetBody, $targetMotionInfo, $targetMotionBody) + @($targetMovies.Values)) {
    if (-not (Test-Path -LiteralPath $file)) {
...
$currentMotionInfoHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $targetMotionInfo).Hash
$currentMotionBodyHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $targetMotionBody).Hash
...
if (($currentMotionInfoHash -eq $ExpectedMotionInfoHash -or $currentMotionBodyHash -eq $ExpectedMotionBodyHash) -and
    (-not (Test-Path -LiteralPath $backupMotionInfo) -or -not (Test-Path -LiteralPath $backupMotionBody))) {
    Write-Fail "Le patch d'interface semble deja installe, mais une sauvegarde originale manque."
    Write-Host "  Verifiez d'abord l'integrite des fichiers du jeu dans Steam." -ForegroundColor Red
    Pause-And-Exit 1
}
...
if (Test-Path -LiteralPath $backupMotionInfo) {
    Write-Ok "Sauvegarde existante conservee : motion_info.psb.m.bak"
} else {
    Copy-Item -LiteralPath $targetMotionInfo -Destination $backupMotionInfo
    Write-Ok "Sauvegarde creee : motion_info.psb.m.bak"
}
if (Test-Path -LiteralPath $backupMotionBody) {
    Write-Ok "Sauvegarde existante conservee : motion_body.bin.bak"
} else {
    Copy-Item -LiteralPath $targetMotionBody -Destination $backupMotionBody
    Write-Ok "Sauvegarde creee : motion_body.bin.bak"
}
foreach ($name in $ExpectedMovieHashes.Keys) {
...
Copy-Item -LiteralPath $frMotionInfo -Destination $targetMotionInfo -Force
Copy-Item -LiteralPath $frMotionBody -Destination $targetMotionBody -Force
foreach ($name in $ExpectedMovieHashes.Keys) {
...
$installedMotionInfoHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $targetMotionInfo).Hash
$installedMotionBodyHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $targetMotionBody).Hash
...
if ($installedMotionInfoHash -ne $ExpectedMotionInfoHash -or $installedMotionBodyHash -ne $ExpectedMotionBodyHash) {
    Write-Fail "La verification des fichiers d'interface installes a echoue."
    Pause-And-Exit 1
}
foreach ($name in $ExpectedMovieHashes.Keys) {
...
Write-Ok "Le scenario, l'interface et les trois videos ont ete installes et verifies."
